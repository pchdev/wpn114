...
void MIDIHandler::setOutPort(uint8_t port)
{
    if ( m_out_port != port )
    {
...
void MIDIHandler::setInPort(uint8_t port)
{
    if ( m_in_port != port )
    {
...
void MIDIHandler::setInDevice(QString device)
{
    if ( m_in_device != device )
    {
        ...
        if ( auto idx = inDevices().indexOf(device) >= 0 )
            setInPort(idx);
    }
}
...
void MIDIHandler::setOutDevice(QString device)
{
    if ( m_out_device != device )
    {
        ...
        if ( auto idx = outDevices().indexOf(device) >= 0 )
            setOutPort(idx);
    }
}
