    ...
    m_device_hdl(std::move(hdl))
{
    light_pad(
              0,
              pad_colors::BLUE_GREEN,
              pad_lighting_mode::FLASH_EIGHTH_BEATS);
}
...
{
    m_device_hdl->out(
                commands::CONTINUOUS_CONTROL,
                static_cast<uint8_t>(mode),
                target,
                static_cast<uint8_t>(color));
}

void push_1::light_cc(command_buttons target, toggle_lighting_mode mode)
{
    m_device_hdl->out(commands::CONTINUOUS_CONTROL, 0,
...
void push_1::light_cc_toggle(uint8_t row, uint8_t target, toggle_lighting_mode mode)
{
    ...
    m_device_hdl->out(commands::CONTINUOUS_CONTROL, 1,
                      target+offset,
                      static_cast<uint8_t>(mode));
}
