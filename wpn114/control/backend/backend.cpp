...
template<typename T> static void device_hdl::print_all_midi_ports() noexcept
{
    T               rt_port_hdl;
...
template<typename T> static int device_hdl::get_port_index(std::string& port_id) noexcept
{
    T           rt_port_hdl;
...
{
    T* res_port         = new T();
    auto port_index     = device_hdl::get_index_for_port<T>(port_id);
...
void device_hdl::out(commands command_id, uint8_t channel, uint8_t index, uint8_t value) const
{
    vector<uint8_t> output = {
        static_cast<uint8_t>(command_id + channel),
        index, value };
